...
# save into rds
rm(list = ls())
# setwd('C:/Users/timot/OneDrive/Augustana/Junior year/Spring semester/DATA 332/Uber')

combined_uber <- readRDS("combined_uber.rds")

# raw_uber_apr <- read.csv("uber-raw-data-apr14.csv")
# 
# raw_uber_may <- read.csv("uber-raw-data-may14.csv")
# 
# raw_uber_jun <- read.csv("uber-raw-data-jun14.csv")
# 
# raw_uber_jul <- read.csv("uber-raw-data-jul14.csv")
# 
# raw_uber_aug <- read.csv("uber-raw-data-aug14.csv")
# 
# raw_uber_sep <- read.csv("uber-raw-data-sep14.csv")
# 
...
# 
# combined_uber$Base <- as.factor(combined_uber$Base)
# 
...
pivot_hour <- combined_uber %>%
  group_by(hour) %>%
  summarise(count = n()) %>%
  arrange(hour)

pivot_hour_month <- combined_uber %>%
  group_by(hour, Month) %>%
  summarize(count = n())

pivot_day <- combined_uber %>%
  group_by(Day) %>%
  summarize(count = n())

pivot_day_of_week <- combined_uber %>%
  group_by(Month, DayOfWeek) %>%
  summarize(count = n())

pivot_hour_day <- combined_uber %>%
  group_by(hour, Day) %>%
  summarize(count = n())

day_order <- c("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday")

pivot_day_of_week$DayOfWeek <- factor(pivot_day_of_week$DayOfWeek, levels = day_order)

pivot_hour_DayOfWeek <- combined_uber %>%
  group_by(hour, DayOfWeek) %>%
  summarize(count = n())

pivot_base <- combined_uber %>%
  group_by(Base, Month) %>%
  summarize(count = n())

pivot_month_day <- combined_uber %>%
  group_by(Month, Day) %>%
  summarize(count = n())

pivot_month_week <- combined_uber %>%
  group_by(Month, week_number) %>%
  summarize(count = n())

pivot_base_DayOfWeek <- combined_uber %>%
  group_by(Base, DayOfWeek) %>%
  summarize(count = n())

pivot_base_DayOfWeek$DayOfWeek <- factor(pivot_base_DayOfWeek$DayOfWeek, levels = day_order)

pivot_month <- combined_uber %>%
  group_by(Month) %>%
  summarize(count = n())
...
rides_per_day[row_index, column_name] <- rides_per_day[row_index, column_name] * 2

rides_per_day$Day <- as.numeric(as.character(rides_per_day$Day))

lm_model_day <- lm(rides_per_day$rides ~ rides_per_day$Day)

# Extract slope and intercept
intercept_day <- coef(lm_model_day)[1]
slope_day <- coef(lm_model_day)[2]
...
ggplot(rides_per_day, aes(x = Day, y = rides)) +
  ...
  labs(title = "Number of Rides per Day", x = "Day", y = "Number of Rides")



rides_per_month <- combined_uber %>%
  group_by(Month) %>%
  summarise(rides = n())

rides_per_month <- rides_per_month %>%
  separate(Month, into = c("Month_Number", "Month_Name"), sep = " ")

rides_per_month <- rides_per_month %>%
  mutate(Month_Number = as.integer(Month_Number))

rides_per_month$Month_Number <- as.numeric(as.character(rides_per_month$Month_Number))

lm_model_month <- lm(rides_per_month$rides ~ rides_per_month$Month_Number)

# Extract slope and intercept
intercept_month <- coef(lm_model_month)[1]
slope_month <- coef(lm_model_month)[2]
...
server <- function(input, output) {
  ...
  output$predictive_chart2_desc <- renderText({
    "Description: This is scatter plot with a regression line that predicts how the number of rides will evoluate when grouped by day of the month. We can see that the slope is slightly positive which means that people tends to use uber more at the end of the month."
  })
